...
1. 批次處理（建議）
   ...
   第 4 欄：config.yaml 內的伺服器名稱（例如 LK, LNK）
           若搭配 --lookup 可省略此欄位（系統會依 lookup 順序嘗試）
...
5. 進階選項
   ...
   --lookup all     依序嘗試所有伺服器（會自動帶入 LK -> LK1 等數字備援）
   --lookup LK TY   依序嘗試指定伺服器（同樣會自動帶入數字備援）
   --alt-modality CR US  指定替代模態（當 CT/MR 不一致時依序查詢）
...
2. 查無資料：
   ...
   - 確認日期格式（YYYY-MM-DD）
   - 確認模態縮寫（CT, MR, MG 等）
   - 模態可能不同時，使用 --alt-modality 嘗試其他縮寫
...
- 可使用 --lookup 依序嘗試不同伺服器（並自動嘗試 LK1/TY1 等數字備援）
